...
async function startServer() {
  ...
  const webhookRateLimit = rateLimit({
    ...
    keyGenerator: (req) => {
      ...
      // ipKeyGenerator normaliza IPv6 para evitar ValidationError do express-rate-limit v8
      const headerToken = req.headers['x-webhook-token'] || req.headers['authorization'];
