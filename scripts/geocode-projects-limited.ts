...
async function main() {
  ...
  // Buscar apenas 50 projetos sem coordenadas para teste
  const projectsWithoutCoords = await db
    .select()
    .from(projects)
    .where(or(isNull(projects.latitude), isNull(projects.longitude)))
  ...
  console.log(`Found ${projectsWithoutCoords.length} projects to geocode (limited to 50 for testing)`);
  ...
  for (const project of projectsWithoutCoords) {
    ...
    console.log(`Geocoding project ${project.id}: ${project.nome}`);
