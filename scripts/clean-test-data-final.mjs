/**
 * Script de limpeza de dados de teste - Seu Metro Quadrado CRM (final)
 * Critério de remoção: usuários com email @test.com, @teste.com ou @exemplo.com
 */
...
const [testUsers] = await conn.execute(`
  ...
  WHERE email LIKE '%@test.%' OR email LIKE '%@exemplo.com%' OR email LIKE '%@teste.com%'
  ORDER BY name
`);
...
// Leads adicionais de teste (por nome/email)
